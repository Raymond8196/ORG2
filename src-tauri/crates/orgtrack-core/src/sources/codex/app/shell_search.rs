...
pub(crate) fn normalize_tool_name_key(raw_name: &str) -> String {
    raw_name
        .trim()
...
pub(crate) fn rg_search_args_from_shell_args(shell_args: &Value) -> Option<Value> {
...
pub(crate) fn exploration_tool_calls_from_shell_args(shell_args: &Value) -> Option<Vec<(String, Value)>> {
...
pub(crate) fn shell_args_for_command_part(shell_args: &Value, command: &str) -> Value {
...
pub(crate) fn is_exploration_context_probe(tokens: &[String]) -> bool {
...
pub(crate) fn rg_files_args_from_shell_args(shell_args: &Value) -> Option<Value> {
    let command = shell_args.get("command").and_then(Value::as_str)?.trim();
    let tokens = shell_tokens(command);
    if !tokens.first().is_some_and(|token| is_rg_executable(token))
...
pub(crate) fn find_args_from_shell_args(shell_args: &Value) -> Option<Value> {
    let command = shell_args.get("command").and_then(Value::as_str)?.trim();
    let tokens = shell_tokens(command);
...
pub(crate) fn option_values(tokens: &[String], short: &str, long: &str) -> Vec<String> {
...
pub(crate) fn has_only_output_limiter_pipeline(tokens: &[String]) -> bool {
...
pub(crate) fn push_shell_token(tokens: &mut Vec<String>, current: &mut String) {
    if current.is_empty() {
        return;
    }
...
pub(crate) fn is_shell_separator(token: &str) -> bool {
    matches!(token, "&&" | "||" | ";" | "|")
}

pub(crate) fn is_rg_executable(token: &str) -> bool {
    let executable = token.rsplit('/').next().unwrap_or(token);
    matches!(executable, "rg" | "ripgrep" | "grep")
}

pub(crate) fn rg_pattern_from_tokens(tokens: &[String]) -> Option<String> {
    let mut index = 0usize;
...
pub(crate) fn rg_flag_consumes_next(token: &str) -> bool {
    matches!(
        token,
        "-g" | "--glob"
            | "-t"
