...
fn ingests_http_origin_and_reports_change() {
    let first = ingest_advertised_url("folder-1", "http://localhost:5173/")
        .expect("parse ok");
    assert!(first.0);
    assert_eq!(first.1, 5173);

    let second = ingest_advertised_url("folder-1", "http://localhost:5173")
        .expect("parse ok");
    assert!(!second.0);
...
fn strips_trailing_punctuation() {
    let result = ingest_advertised_url("folder-1", "https://127.0.0.1:3000/.")
        .expect("parse ok");
