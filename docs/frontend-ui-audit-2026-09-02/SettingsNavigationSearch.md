...
| `src/scaffold/NavigationSidebar/variants/SettingsSidebar.tsx:419`                      | Persistent Settings search      | keep with reason | Reuses the design-system `Input` and shared dropdown; it remains fixed, left-aligned, and closed until input is typed.                              | None.            |
...
- Focused Vitest covers all-schema parity, global localized `皮肤` and `强调色` matches from another mounted page, MCP navigation, row-key resolution, delayed rendering, scrolling, focus, keyboard selection, Escape dismissal, and empty-query behavior
