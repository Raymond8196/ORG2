...
| ------------------------------------- | ------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `SessionCreatorChatPanelView.tsx:281` | CLI version alert               | keep with reason | Reuses `InlineAlert`, the existing neutral alert surface, and a semantic upgrade icon. Copy stays on one line and all actions remain in the alert header. | None.            |
| `SessionCreatorChatPanelView.tsx:287` | Suppression and refresh actions | keep with reason | Reuses shared `Button` variants and sizes; the refresh control has a localized accessible label and the text action is self-describing.                   | None.            |
