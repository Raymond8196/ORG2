...
| ------------------------------------- | ------------------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| `SessionCreatorChatPanelView.tsx:287` | CLI version alert               | keep with reason | Reuses `InlineAlert`, the existing neutral alert surface, and a semantic upgrade icon. Copy stays on one line and all actions remain in the alert header. | None.            |
| `SessionCreatorChatPanelView.tsx:294` | Suppression and refresh actions | keep with reason | Reuses shared icon-only `Button` variants and sizes; both controls have localized tooltips and accessible labels.                                         | None.            |
