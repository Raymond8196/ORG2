# Ops Control GitHub lazy lists and cache — architecture audit

## Acceptance criteria

- Ops Control fetches open pull requests on entry and fetches closed pull requests only when the Closed or Merged view is selected.
...
- Ops Control retains only one active page/query snapshot per GitHub scope and evicts it on access after 10 minutes.
- Switching between Issues, PRs, and other Ops Control sections restores the active page without retaining component trees.
...
| 2. Dead code / structural duplication | GitHub list-cache ownership                                 | pass    | The cache moved from a Workstation hook directory to `services/git`; My Station and Ops Control now import the same live implementation.                                                          |
...
| 6. Cross-domain leakage               | Workstation ↔ Ops Control                                   | pass    | Data caching is owned by the shared Git service. Ops-only query/page snapshots remain local to Ops Control and do not leak UI concerns into the service.                                          |
