...
- The Ops Control tab requires full-screen ChatPanel presentation and restores the user's prior maximize state on exit.
- The full-screen management tab keeps the top tab bar and suppresses the Workstation rail.
...
| 7. New-developer clarity              | Entry points and ownership                              | pass             | `openOpsControlChatPanelTabAtom`, `isChatPanelTabFullscreenOnly`, and `openOpsControlTab` describe their responsibility directly.                                                                 |
...
| Entry point                             | Opens singleton tab | Makes chat visible | Requires full screen | Selects Ops home | Normalizes legacy route |
| --------------------------------------- | ------------------: | -----------------: | -------------------: | ---------------: | ----------------------: |
| Shortcut / Action / Spotlight           |                 yes |                yes |                  yes |              yes |                     yes |
| Legacy `/workstation/ops-control` route |                 yes |                yes |                  yes |              yes |                     yes |
| ChatPanel `+` menu                      |                 yes |    already visible |                  yes |              yes |                     n/a |
| Retained Ops sidebar                    |                 yes |    already visible |                  yes |              yes |                     n/a |
