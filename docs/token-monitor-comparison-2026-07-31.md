...
- Token Monitor 有 19 个 quota provider；ORGII 本轮后可直接刷新其中 11 个，剩 8 个。
- Token Monitor 有 21 个 usage client（20 个默认、MiMo opt-in）；ORGII 与其中 9 个 client 有真正 transcript importer，另有 6 个 Token Monitor 没有的 source。Antigravity、Kimi、Qwen 在 ORGII 目前只是 hook/source identity，不是历史 importer。
...
本轮实际扩展了 DeepSeek、OpenRouter、MiniMax、ZAI Team、Qoder quota，加入 Pi importer，把 Cursor billing 改为 streaming aggregate + bounded paging，并为 JSONL importer 建立固定 seam watermark。所有新路径都是 demand-driven，没有新增 interval、常驻 watcher 或后台 worker。
...
| 状态 | Provider |
|---|---|
| ORGII 已支持（11/19） | Claude OAuth、Codex OAuth、OpenCode Go、Cursor、Copilot、ZAI/GLM、DeepSeek、OpenRouter、MiniMax、ZAI Team、Qoder |
| 尚未支持（8/19） | Antigravity、Kimi、Grok、MiMo、Kiro、Volcengine、Ollama Cloud、third-party/NewAPI |
...
ORGII 的 16 个 importer source：

`claude_code`, `codex`, `cursor_cli`, `cursor_ide`, `opencode`, `cline`, `windsurf`, `warp`, `trae`, `zcode`, `qoder`, `qoder_cli`, `mimo_code`, `omp`, `pi`, `workbuddy`。
...
- ORGII 的 Antigravity、Kimi、Qwen metadata 只是 runtime/hook identity，不能算 usage importer。
...
| Kimi | Coding API 与 web membership 多入口 | 缺失 | 先支持 saved Kimi Code API；web cookie 作为显式 profile，禁止自动多路 fan-out |
...
| Kimi | Kimi/Kimi Code 双 JSONL root；config 参与 fingerprint | 只有 hook identity | 共用 JSONL watermark pipeline；双 root 同 client 去重 |
| Qwen | projects JSONL；generic full-message cache | 只有 hook identity | byte watermark + directory snapshot；不整文件重 hash |
...
| P2 | Qwen/Kimi/Proma 尚未接统一 watermark pipeline | provider breadth 仍不完整 | 复用已完成的 seam/watermark 基础设施；每个 source 保持精确 root 与硬 scan cap |
...
11. **未完成** — Kimi Code quota 与 Kimi/Kimi Code importer。
12. **部分完成** — Pi importer 完成并复用 watermark；Qwen 尚未接入。
13. **未完成** — Proma importer。
...
- 还差 8 个 Token Monitor quota provider。
- 还差 12 个 Token Monitor usage client 的完整 importer。
...
- 所有 7 个 PR 当前 merge state 为 clean；#611/#614 Ready 且全 CI 通过，5 个 stacked PR 保持 Draft。
