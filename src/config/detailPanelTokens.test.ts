...
import {
  ...
  ISSUE_DETAIL_PANEL_WIDTH_TOKENS,
...
describe("detail panel width tokens", () => {
  it("keeps standard and issue widths as separate contracts", () => {
    ...
    expect(ISSUE_DETAIL_PANEL_WIDTH_TOKENS.headerWidth).toContain(
      "max-w-[1232px]"
    );
    expect(ISSUE_DETAIL_PANEL_WIDTH_TOKENS.contentMaxWidth).toContain(
      "max-w-[1200px]"
    );
