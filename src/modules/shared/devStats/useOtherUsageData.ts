...
import type { ProfileDateRange } from "./config";
import {
  DEFAULT_TAB,
...
export interface UseOtherUsageDataReturn {
  ...
  // Model breakdown metric toggle ($ vs tokens)
  modelBreakdownMetric: ModelBreakdownMetric;
  setModelBreakdownMetric: (metric: ModelBreakdownMetric) => void;
  ...
  modelChartData: { model: string; tokens: number; cost: number }[];
...
export function useOtherUsageData(): UseOtherUsageDataReturn {
  ...
  // Default to estimated cost — these are token-only sources with no metered spend.
  const [modelBreakdownMetric, setModelBreakdownMetric] =
    useState<ModelBreakdownMetric>(MODEL_BREAKDOWN_METRIC.COST);
